  ...
  try {
    ...
    if (challenge) {
      ...
      setPushToTalkState({
        ...
        isListening: false,
        isBusy: false,
        lastError: "Remote transcription is paused for privacy permission. No microphone audio was sent; the backend stopped and discarded active capture. Review the privacy request, then repeat the voice input.",
      });
      await refreshRuntimePanels();
      return;
    }
