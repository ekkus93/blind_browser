...
- Remote ASR privacy evaluation now happens before top-level microphone capture. Consent-required pending state stores metadata only; active capture is stopped/discarded on privacy rejection; approved microphone consent requires repeating the voice input instead of replaying pre-consent audio. The remote ASR provider/profile/endpoint snapshot is taken with authorization before the unlocked capture window.
- Fixed narration `Allow once` so the one-shot grant is bound to and consumed by the exact narration payload retry rather than being consumed before the resumed policy evaluation. Added corresponding narration/microphone one-shot and fail-closed evidence.
- Completed frontend consent routing and speech-specific copy for narration/microphone disclosures, plus TTS/ASR settings privacy-mode/site-rule/notices. Added `submit_microphone_consent_response` to direct-command semantic evidence.
- Permanent CI exposed and correctly failed on two evidence-maintenance drifts during closure: the direct-command evidence inventory initially omitted `submit_microphone_consent_response`, and `ISOLATED_WRY_TESTS` initially omitted two newly `#[ignore]`d privacy tests. Both inventories were repaired; neither guard was weakened or suppressed.
- Exact validated implementation SHA: `15b1f5890b17722ab126c97acdc6a050168a108d`. Permanent CI run `31277834628`, job `93154218640`: SUCCESS across privacy/fallback scanners, rustfmt, default cargo check, strict Clippy `-D warnings`, direct-command semantic evidence, complete Rust/Wry tests, frontend lint/UI tests, and production build.
- P1.2 remains explicitly BLOCKED: planner-embedded TTS/ASR still execute inside the executor-wide `AppCore` lock; fixing that requires a larger pausable/resumable plan-execution refactor. CR3 completion permits an explicitly evidenced blocker, so no false closure claim is made.
...
## 2026-08-07T23:50:10Z - Claude Sonnet 5 - CR3 P2 and P3 phases complete (Ralph loop continuation)
- Continued the CR3 Ralph loop (P0/P1 already done in a prior session) through P2.1-P2.8 and P3.1-P3.3. All items DONE, `docs/BB_CODE_REVIEW3_TODO.md` updated with detailed implementation notes per item (including several honest divergences from the review's literal wording, caught by verification). Commits d21d07e..adc9e5b on master.
- P2.1-P2.4: click-authorization subsystem evidence test; `ISOLATED_WRY_TESTS` self-verifying completeness guard in `run-rust-tests-linux.sh`; config-file/dir permission hardening (0600/0700) + parent-dir fsync in `atomic_file.rs`; Unicode-aware sensitive-content detection (confusable-ASCII/homoglyph folding, hand-rolled since NFKC does NOT fold cross-script homoglyphs — empirically verified before implementing).
- P2.5: fixed `SanitizationMetadata`'s double-counting bug (hidden elements counted in both `omitted_hidden_elements` and the old `relevance_filtered_elements`); collapsed the now-always-equal `relevance_filtered_elements`/`relevance_filtered_regions` fields out of the struct entirely rather than just fixing the arithmetic.
- P2.6: reproduced the "Allow always" (`AllowPersistent`) planning-snapshot invalidation exactly as the review described (extended the existing policy-matrix isolated-Wry test through execution). First fix attempt (exclude `remote_planner_privacy` from `relevant_config_fingerprint`) was WRONG — the full isolated-Wry sweep caught it breaking a different pre-existing invariant (mode-changed-while-challenge-pending detection shares the same fingerprint function). Reverted; correct fix was re-capturing the planning snapshot in `AllowPersistent`'s arm after `persist_origin_rule` runs, touching nothing shared.
- P2.7: process-level `OnceLock<Mutex<Option<CachedWhisperContext>>>` cache in `asr/local.rs` (free-standing, not a controller field, preserving the CR2 lock-scoping property that `transcribe_local` holds no state).
- P2.8: `ValidatedPlannerOutput` newtype makes `PlannerResolution::Direct` require validation by construction, not convention (closes the "14th resolver forgets validation" class of bug); `AppConfig::mutate_config_document` + `table_mut`/`profile_table_mut` collapse the 10 near-identical config persisters' shared load/write/reload skeleton; `FOCUS_RING`/`DISMISS_BUTTON_CLASS` consolidated into `shared-controls.tsx`; removed 3 of 4 named dead-code items (kept `renderSettingsProviderFailoverPanelNode` — its own comment says it's deliberately unwired pending a not-yet-built backend feature, not cruft; verification changed the outcome).
- P3.1: replaced `docs/SPECS.md`'s drifted "Exact initial example contents" TOML block with a pointer to `config.example.toml`; implemented every documented-but-unenforced validation rule (`always_confirm_submit`, remote-profile `timeout_ms`/`max_output_tokens`/`temperature_milli`/`base_url`, local-profile `model_path`/`threads`/`sample_rate`) rather than retracting any — checking "for each rule in the section" surfaced 2 more unenforced rules than the review's own short list named.
- P3.2: `high_risk_origin_policy` had one enum variant and was never read by the policy evaluator — a genuinely open decision (wire it up with real bypass variants vs. retract), touching CLAUDE.md's own Design-choice locks section, so this was the one item surfaced to the user via AskUserQuestion rather than decided unilaterally. User chose retract. Removed end-to-end (Rust + TS, ~22 files); added a dated removal plan (tied to the next `REMOTE_DATA_POLICY_VERSION` bump) for the three related legacy privacy fields the review flagged as at-risk of becoming permanent, without removing them in this pass.
- P3.3: three pre-existing (not migration-caused) visual/a11y bugs — unclassed headings (added `SETTINGS_SECTION_HEADING_CLASS`/`SETTINGS_CARD_HEADING_CLASS`, found and fixed one extra instance the review didn't name); guidance-CTA contrast (computed WCAG contrast against the gradient's actual worse-case end color — the established `#f6f2eb` convention would have only cleared ~4.43:1 here, used `#fffdf8` for ~4.93:1); permanent empty error strip (the `"none"` variant's class string, not the always-mounted container, was the bug).
- Ralph-loop discipline held throughout: full validation (fmt/clippy/cargo test/4 guard scripts/xvfb/isolated-Wry sweep for Rust; lint/build/test:ui for frontend) after every phase, one commit per phase, TODO doc updated before each commit.
- Final state: 551 Rust tests (9 isolated-Wry, run via `run-rust-tests-linux.sh`), 250 JS tests, clippy/lint/build all clean. Remaining open item from an earlier session: P1.1 follow-up (gate remote ASR/microphone audio through the shared consent layer — groundwork already in place, deferred pending `execute_transcribe_command` phase-splitting).
