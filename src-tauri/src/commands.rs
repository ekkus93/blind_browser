...
mod tests {
    ...
    fn browser_history_navigation_updates_following_state_reads() {
        ...
        let runtime_after_back_result =
            execute_planned_step(&mut executor, &runtime_after_back_step);
        ...
        let agent_after_reload_result =
            execute_planned_step(&mut executor, &agent_after_reload_step);
...
Use OCR threshold tuning when extraction fails."#,
    ...
    fn tool_result_success_populates_common_envelope_fields() {
        ...
        assert_eq!(result.observations, vec![String::from("runtime status read")]);
        assert_eq!(
            result
                .data
                .as_ref()
    ...
    fn tool_result_failure_populates_common_envelope_fields() {
        ...
        assert!(result.warnings.is_empty());
        assert_eq!(result.observations, vec![String::from("volume update failed")]);
        assert_eq!(
            result.error,
            Some(ToolError {
                code: String::from("audio_update_failed"),
