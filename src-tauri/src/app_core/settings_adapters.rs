...
pub(crate) fn build_remote_tts_settings(config: &AppConfig) -> RemoteTtsSettings {
    ...
    RemoteTtsSettings {
        ...
        privacy_notice: String::from(
            "Remote narration sends narration text to the configured TTS endpoint only after the independent remote narration privacy policy authorizes it.",
        ),
    }
}
...
pub(crate) fn build_remote_asr_settings(config: &AppConfig) -> RemoteAsrSettings {
    ...
    RemoteAsrSettings {
        ...
        privacy_notice: String::from(
            "Remote voice input sends microphone audio to the configured ASR endpoint only after the independent microphone privacy policy authorizes capture and disclosure.",
        ),
    }
}
