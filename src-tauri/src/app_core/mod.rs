...
pub struct AppCore {
    ...
    // kind. Planner context, narration text, and microphone audio are materially
    // different disclosures, so permission for one must never cross-authorize
    // another. All three stores use the same fail-closed policy evaluator.
...
mod narration;
pub(crate) mod remote_data_consent;
mod remote_planner;
