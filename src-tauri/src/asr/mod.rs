...
impl AsrController {
    ...
    pub(crate) fn transcribe_command(
        ...
        if matches!(config.providers.asr.mode, ProviderMode::Remote)
            && remote_authorization.is_none()
        {
            // Fail before capture: an unauthorized remote request must not even
            // start a microphone window and then discover the missing capability
            // only when it reaches the network provider.
...
mod tests {
    ...
    #[test]
    fn remote_asr_transcribe_rejects_before_capture_without_authorization() {
        let mut config = AppConfig::default();
        config.providers.asr.mode = ProviderMode::Remote;
        let mut controller = super::AsrController::new();

        let error = controller
            .transcribe_command(&config, 1, true, None)
            .expect_err("remote ASR must reject before microphone capture without authorization");

        assert!(matches!(error, AsrRuntimeError::RemoteConsentMissing));
        assert!(
            !controller.is_listening(),
            "missing remote consent must not leave a capture session active"
        );
    }
