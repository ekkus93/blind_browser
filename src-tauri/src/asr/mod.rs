...
use crate::app_core::RemoteMicrophoneAuthorization;
...
pub(crate) fn transcribe_captured_audio(
    ...
    match config.providers.asr.mode {
        ...
        ProviderMode::Remote => {
            let _authorization =
                remote_authorization.ok_or(AsrRuntimeError::RemoteConsentMissing)?;
...
mod tests {
    ...
    fn remote_asr_dispatch_fails_closed_without_consent_authorization() {
        ...
        let error = transcribe_captured_audio(&config, &captured_audio, None).expect_err(
            "remote ASR must reject dispatch without authorization before provider setup",
        );
