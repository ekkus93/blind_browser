...
pub(super) fn validate_audio_settings(audio: &AudioSettings, issues: &mut Vec<String>) {
...
pub(super) fn validate_safety_settings(safety: &SafetySettings, issues: &mut Vec<String>) {
...
pub(super) fn validate_ocr_settings(ocr: &OcrSettings, issues: &mut Vec<String>) {
...
pub(super) fn validate_model_settings(models: &ModelManagementSettings, issues: &mut Vec<String>) {
    if models.models_dir.trim().is_empty() {
        issues.push(String::from("models.models_dir must not be empty"));
    }
}

pub(super) fn normalize_remote_endpoint(base_url: &str) -> Result<String, ConfigError> {
