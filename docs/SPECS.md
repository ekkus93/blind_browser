...
- also trigger OCR when extracted text is sparse, with defaults of `200` readable characters or fewer than `2` readable regions
...
- `region_id`, when provided, must resolve to a page region with a positive stored bounding box.
- `region_id` targeting still requires `image_id` so OCR has an explicit cached screenshot source.
...
- Mixed commands such as `fill the email field and then submit` should normalize to the `SubmitForm` family so later planning can preserve both the fill and submit workflow; when the target field grounds deterministically from the current page model, the runtime may shortcut directly to a confirmation-gated `confirm_action` → `focus_element` → `type_into_element` → `submit_active_form` sequence instead of invoking the planner.
- Focus-field phrases such as `focus the email field` should normalize to `FillInput`; when a single visible field-like control can be grounded deterministically from the current page model, the runtime may shortcut directly to a bounded `focus_element` action instead of invoking the planner.
- Fill-field phrases such as `fill the email field with phil@example.com` and `type hello into the search field` should remain in the `FillInput` family; when the target field grounds deterministically from the current page model, the runtime may shortcut directly to bounded `focus_element` plus `type_into_element` steps without planner fallback.
- Submit-form phrases such as `submit form` and `submit this form` should normalize to `SubmitForm`; when there is a current page and the form target is unique or can be resolved as the active form, the runtime may shortcut directly to a confirmation-gated `submit_active_form` step sequence instead of invoking the planner.
